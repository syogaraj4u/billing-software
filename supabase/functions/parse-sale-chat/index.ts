...
Deno.serve(async request => {
  ...
  try {
    ...
    const { message, profiles = [], profileAliases = [], partyAliases = [], items = [], parties = [], activeProfileId = "" } = await request.json();
      ...
      input: [
        ...
        },
        {
          ...
          content: [
            {
              type: "input_text",
              text: JSON.stringify({ message, activeProfileId, profiles, profileAliases, partyAliases, items, parties })
            }
          ]
        }
      ],
