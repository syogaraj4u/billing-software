  ...
  properties: {
    ...
    lines: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["name", "hsn", "qty", "rate", "gstRate", "imeiNumbers"],
        properties: {
          name: { type: "string" },
          hsn: { type: "string" },
          qty: { type: "number" },
          rate: { type: "number" },
          gstRate: { type: "number" },
          imeiNumbers: { type: "string", description: "Newline-separated IMEI or serial numbers belonging to this item row. Keep empty string when none are visible." }
        }
      }
    }
  }
...
Deno.serve(async request => {
  ...
  try {
    ...
    const { fileName, mimeType, base64, profiles = [], supplierProfiles = [] } = await request.json();
      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: "Extract an Indian GST purchase invoice into a purchase register draft. The buyer must be one of the provided GST profiles when possible. Use the provided supplierProfiles as supplier-specific extraction hints. When supplier GSTIN/name matches a supplier profile, use its default address, place, and PIN only when that data is missing or unreadable in the invoice. invoiceDate must be the Tax Invoice Date, Invoice Date, or Dated value associated with the invoice number. Never use ACK Date, acknowledgement date, IRN date, e-way bill generated date, delivery date, dispatch date, document date, generated date, or validity date as invoiceDate. Recognize all common invoice date formats, including DD/MM/YYYY, DD-MM-YY, DD.MM.YYYY, YYYY-MM-DD, YYYY/MM/DD, DD-MMM-YY, DD Month YYYY, Month DD YYYY, compact DDMMYYYY, compact YYYYMMDD, compact DDMMMYYYY, ordinal dates, and invoice date/time values. Interpret ambiguous numeric dates as Indian day-first dates. Return invoiceDate only as YYYY-MM-DD. If the invoice date is missing or unreadable, return an empty string and add a review message instead of using another date. Extract supplier and buyer address/place when visible; return an empty string when not visible. Keep any visible six digit PIN code inside supplierAddress and buyerAddress. Return clean catalog item names: for Apple/iPhone products keep only product family, model/variant, and storage, for example 'Apple iPhone 17 256GB Blue SKU ABC123' becomes 'iPhone 17 256GB' and 'IPHONE 17 PRO MAX 512 NATURAL TITANIUM' becomes 'iPhone 17 Pro Max 512GB'. Remove colors, SKU/item/model codes, EAN, IMEI, serial, batch, and other supplier-only suffixes from item names. Put every visible IMEI or serial number for an item into that line's imeiNumbers field as newline-separated values. Never include IMEI/serial values inside line.name. Separate CGST, SGST and IGST exactly as shown. If an item HSN/SAC is not visible, partial, or printed as 8517 for mobile phones, use default HSN 85171300. Never return HSN 85176290. If payment lines show card cashback, cashback, debit note, or any post-total payment adjustment, return it in roundOff as a negative number; for example CARD CASH BACK - 6000 means roundOff -6000. Keep extractedTaxes.total as the invoice total before cashback, while total may be the net debited/payable total. For invoices that say values are inclusive of GST, including Reliance/Reliance Digital receipts, return each line rate as taxable value before GST, not MRP or paid gross amount, and set total to the net paid/invoice total. For CELL9 MOBILE STORE invoices, the RATE column is taxable unit rate and the printed AMOUNT/TOTAL is the grand total inclusive of GST; for example Qty 5, Rate 49,576.27, CGST 22,309.32, SGST 22,309.32 means taxable 247,881.35, gst 44,618.64, total 292,500.00, and line rate 49,576.27. For MANGO MOBILES scanned invoices, supplier GSTIN is 33CHUPM4245C1ZB, invoice number appears near 'Invoice no:', invoice date appears near 'Date of Invoice', Price is taxable unit rate, Amount is GST-inclusive line total, two 9% tax columns together equal 18% tax, and IMEI numbers under each product description belong to that item line's imeiNumbers field and are not amounts. If Mango supplier state differs from buyer state, return the two 9% columns combined under IGST for GST breakup. For HARSHITH ENTERPRISES / Infinite Apple Authorised Reseller invoices, the item table Taxable Value is not the invoice total; use CGST Amount + SGST Amount/IGST Amount plus taxable to calculate total, and prefer the printed Net Amount as invoice total. Example: taxable 4,32,203.39, CGST 38,898.31, SGST 38,898.31, Net Amount 5,10,000.00 means extractedTaxes.taxable 432203.39, gst 77796.62, total 510000.00, and line grossRate 102000 for qty 5. Do not use IMEI numbers, time, bill number, or the watermark as amounts. If an invoice has a GST receipt summary by HSN, prefer that summary for taxable, tax, and total values. Add reviewMessages for unclear buyer, supplier, GSTIN, address, unreadable values, payment adjustment, or tax mode mismatch."
            }
          ]
        },
        {
          ...
          content: [
            ...
            {
              type: "input_text",
              text: JSON.stringify({ fileName, profiles, supplierProfiles })
            }
          ]
        }
      ],
